...
const SESSION_KEY = 'growin_staff_session_dashboard_live_v3';
const BUILD_TAG = 'Growin Dashboard Live v3';
...
function weatherIconName(row) {
  const s = `${first(row.icon_key, row.icon, row.condition, row.weather_label, row.status)}`.toLowerCase();
  if (s.includes('thunder') || s.includes('storm')) return 'thunderstorm-outline';
  if (s.includes('rain') || s.includes('drizzle') || s.includes('shower')) return 'rainy-outline';
  if (s.includes('snow')) return 'snow-outline';
  if (s.includes('sun') || s.includes('clear')) return 'sunny-outline';
  if (s.includes('night') || s.includes('moon')) return 'moon-outline';
  if (s.includes('cloud') || s.includes('overcast')) return 'partly-sunny-outline';
  if (s.includes('mist') || s.includes('fog') || s.includes('haze')) return 'cloudy-outline';
  return 'partly-sunny-outline';
}
function weatherIconColor(row) {
  const s = `${first(row.icon_key, row.icon, row.condition, row.weather_label, row.status)}`.toLowerCase();
  if (s.includes('rain') || s.includes('drizzle') || s.includes('shower')) return '#1890FF';
  if (s.includes('storm') || s.includes('thunder')) return '#7C3AED';
  if (s.includes('sun') || s.includes('clear')) return '#F59E0B';
  if (s.includes('snow')) return '#38BDF8';
  return '#94A3B8';
}
